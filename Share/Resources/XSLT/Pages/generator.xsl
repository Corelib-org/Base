	...
	<xsl:template match="content" mode="xhtml-content">
		...
		<form method="get">								
			<div>
				...
				<select class="select" name="object">
					<option value="">[Select class tree]</option>
					<xsl:for-each select="code-generator[1]/*">
						<option value="{@name}"><xsl:value-of select="name()"/> -> <xsl:value-of select="@name"/></option>
					</xsl:for-each>
				</select>
		...
		<xsl:if test="/page/settings/get/object != ''">
			<form method="get">
				<input type="hidden" name="object" value="{/page/settings/get/object}"/>
				<input type="hidden" name="write" value="true"/>
				<xsl:for-each select="code-generator[2]/generator[count(files/file) > 0]">
