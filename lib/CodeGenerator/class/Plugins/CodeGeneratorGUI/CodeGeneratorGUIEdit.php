...
//*****************************************************************//
//****************** CodeGeneratorGUIEdit class *******************//
//*****************************************************************//
/**
 * CodeGenerator gui edit view
 *
 * @author Steffen Sørensen <ss@corelib.org>
 * @package Base
 * @subpackage CodeGenerator
 ...
 * @since Version 5.0
 */
class CodeGeneratorGUIEdit extends CodeGeneratorGUIFileXSL {


	//*****************************************************************//
	//**************** CodeGeneratorGUIEdit methods *******************//
	//*****************************************************************//
	/**
	 * create new instance of CodeGeneratorGUIEdit.
	 *
	...
	public function __construct(CodeGeneratorTable $table, DOMElement $settings=null, $prefix=null, $group=null){
		...
		if($this->settings->getAttribute('action') == 'default'){
			...
			$filename = 'edit-'.$this->settings->getAttribute('action');
		}

		$this->_setFilename($prefix.$this->getTable()->getTableReadableVariableName().'/'.$filename.'.xsl');
		$this->_loadContent(CORELIB.'/Base/Share/Generator/Edit.xsl.generator');
	}
	...
	/**
	 * Check to see if a edit field allready exists.
	 *
	 * This method may be overwritten to support a
	 * new implementation of {@link CodeGeneratorGUIEdit::writeField()}
	 *
	 * @see CodeGeneratorGUIEdit::writeField()
	 ...
	 *
	 * @see CodeGeneratorGUIEdit::hasField()
	 * @param CodeGeneratorCodeBlockXML $block
	 * @param CodeGeneratorColumn $column
	...
	public function writeField(CodeGeneratorCodeBlockXML $block, CodeGeneratorColumn $column){
		...
		$param->setAttribute('select', '/page/settings/get/'.$column->getFieldReadableVariableName().'|'.$column->getTable()->getTableReadableVariableName().'/@'.$column->getFieldReadableVariableName());
